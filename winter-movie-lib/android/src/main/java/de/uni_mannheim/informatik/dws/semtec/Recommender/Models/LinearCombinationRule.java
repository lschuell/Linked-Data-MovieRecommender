...
public class LinearCombinationRule {
    ...
    public static Movie[] runLCRule(String identifier) throws Exception {
        // loading data
        HashedDataSet<Movie, Attribute> movies = new HashedDataSet<>();
        new CSVMovieReader().loadFromCSV(new File("data/data/de.uni_mannheim.informatik.dws.semtec/moviesI.csv"), movies);
        ...
        Movie testMovie = movies.getRecord(identifier);
        ...
        Collections.sort(correspondencesList, (Comparator<Correspondence>) (o1, o2) -> {
            return -Double.compare(o1.getSimilarityScore(), o2.getSimilarityScore());
        });

        Movie[] topResults = new Movie[10]; int i = -1;
        ...
        for (Object elem: correspondencesList) {
            ...
            if(i != -1) {
                ...
                topResults[i] = x.getSecondRecord();
                System.out.println(topResults[i].getName());
            }
